...
int main(int argc, char* argv[]){
	...
	set<int> readLineNum;			// 行数集合 [8/27/2015 wangxy]
	vector<int> readClumNum;		// 行数个0 (用于标记) [8/27/2015 wangxy]
	int deep = 0;
...
int DecisionTree::pretreatment(string filename, set<int>& readLineNum, vector<int>& readClumNum)
{
	...
	while(iss >> attr)
	{
		// 存放表头 [8/31/2015 wangxy]
		attributes* s_attr = new attributes();
		s_attr->attriName = attr;
		//初始化属性名
		m_vStatTree.push_back(s_attr);
		//初始化属性映射
		m_mAttr_clum[attr] = m_nAttriNum;
		m_nAttriNum++;
		//初始化可用属性列
							...
							set<int>& readLine, vector<int>& readClumNum)
{
	...
	for (set<int>::iterator line_iter = readLine.begin(); line_iter != iter_end; ++line_iter)
	{
		...
		if (infos[*line_iter][m_nAttriNum - 1] == "yes")
		{
			decisLine = true;
			deciNum++; //无用，因为子树的和就是他，在计算时自动就加完这个数了
		}
		//如果该列未被锁定并且为属性列，进行统计
		for (int i = 0; i < m_nAttriNum - 1; i++)
		{
			if (readClumNum[i] == 0)
			{
