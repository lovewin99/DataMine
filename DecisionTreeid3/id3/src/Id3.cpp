...
{  
	Map1 y; // <属性，数量> [8/25/2015 wangxy] 
	Map2 x;  // <层，<层名，数量>> [8/25/2015 wangxy]
	Map3 xy;  // <层，<层名，<属性，数量>>> [8/25/2015 wangxy]
...
//		ans += t * log2(t); 
		ans += t * ((double)log(t) / (double)log(2.0));
	}  
	return -ans;  
}  
...
{  
	// att_val <层名，数量> [8/25/2015 wangxy]
	// val_cls <层名，<属性，数量>> [8/25/2015 wangxy]
	// s 属性size
	// entropy 信息熵
	double gain = entropy;  
